<?php
    ...
    if(isset($_POST['add_member'])) 
    {
        ...
        else if($img_r == 'upb_failed'){
            echo $img_r;
        }
        else {
            $q = "INSERT INTO `team_details_bis`(`name`,`picture`) VALUES (?,?)";
    ...
    {
       $res =  selectAll('team_details_bis');
    ...
    if(isset($_POST['rem_member']))
    {
        ...
        $pre_q = "SELECT * FROM `team_details_bis` WHERE `id`=?";
        $res = select($pre_q,$values,'i');
        $img = mysqli_fetch_assoc($res);

        if(deleteImage($img['picture'],ABOUT_FOLDER)){
            $q = "DELETE FROM `team_details_bis` WHERE `id`=?";
